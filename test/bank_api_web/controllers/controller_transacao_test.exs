defmodule BankApiWeb.ControllerTransacaoTest do
  ...
  alias BankApi.Schemas.{Transacao, Usuario, Operacao}
  import BankApi.Factory

  test "quando todos parametros estão ok, cria transacao no banco", %{conn: conn} do
    IO.puts"usuario1"
    %Usuario{id: id_origem} = insert(:usuario, email: "tarcisio1@tarcisio.com")
    |> IO.inspect()
    IO.puts"usuario2"
    %Usuario{id: id_destino} = insert(:usuario,  email: "tarcisio2@tarcasdisio.com")
    |> IO.inspect()
    IO.puts"Operacao"
    %Operacao{id: id_operacao} = insert(:operacao, nome_operacao: "Transdfasdsferência")
    |> IO.inspect()
    IO.puts"-------"
    # IO.inspect(id_transacao)

    params = %{
      "conta_origem_id" => id_origem,
      "conta_destino_id" => id_destino,
