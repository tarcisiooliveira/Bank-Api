defmodule BankApiWeb.ControllerRelatorioAdministradorTest do
  ...
  describe "TRANSFERENCIAS" do
    ...
    test "assert Transferência - Todos os valores transferidos por determinado email", state do
    ...
    test "assert transferência efetuada por usuario entre datas",
         state do
      params = %{
    ...
    test "assert transferencia -> Todos os valores transferidos entre usuario x -> y entre datas",
         state do
      params = %{
        "periodo_inicial" => "2021-06-13 00:00:01",
        "periodo_final" => "2021-06-17 00:00:01",
        "email1" => state[:valores].email1,
        "email2" => state[:valores].email2
      }
      ...
      assert %{
               "mensagem" => "Total durante detersminado período entre dois usuários.",
               "operacao" => "Transferência",
               "origem" => _email1,
               "destino" => _email2,
               "resultado" => 7500
